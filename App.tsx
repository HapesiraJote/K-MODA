...
import { db, productsCol, onSnapshot, query, orderBy, addDoc, deleteDoc, doc } from './lib/firebase';
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4">
      <div className="secure-access-modal animate-in zoom-in duration-300">
        ...
        <button onClick={onClose} className="mt-4 text-[10px] text-gray-500 uppercase font-bold tracking-widest hover:text-white transition-colors">Anulo</button>
      </div>
    </div>
  );
};
...
const LoadingScreen = ({ brand, color }: { brand: string, color: string }) => (
  <div className="fixed inset-0 bg-black z-[200] flex flex-col items-center justify-center text-center px-6">
    ...
    <div className="text-xl font-black uppercase tracking-[0.5em] text-white">
      <div className="typing-text border-b-2 border-white/20 pb-2">Duke u ngarkuar...</div>
    </div>
  </div>
);
...
const AppContent = () => {
  ...
  const handleAdminAccess = (code: string) => {
    if (code === MASTER_CODE) {
      const adminData = { id: 'admin', email: 'admin@kmoda.com', name: 'Admin K-MODA', isAdmin: true };
      setUser(adminData);
  ...
  return (
    <>
      ...
      <Layout 
        cart={cart} 
        user={user} 
        onLogout={() => { setUser(null); localStorage.removeItem('user'); navigate('/'); }} 
      ...
      >
        <Routes>
          ...
          <Route path="/product/:id" element={<ProductDetail products={products} addToCart={(item) => {
            setCart(prev => {
              const ex = prev.find(i => i.id === item.id && i.selectedSize === item.selectedSize && i.selectedColor === item.selectedColor);
